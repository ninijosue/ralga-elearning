...
if (isloggedin()) {
    ...
    if (theme_space_get_setting('darkmodetheme') == '1') {
        $darkmodetheme = true;
        if (get_user_preferences('darkmode-on')) {
            $extraclasses[] = 'theme-dark';
        }
    }

    // My Courses preferences
    $mycourses_preferences = [
        'mycourses-on' => 'mycourses-on',
        'mycourses-hidden-on' => 'mycourses-hidden-on',
        'mycourses-inprogress-on' => 'mycourses-inprogress-on'
    ];

    // Add classes based on My Courses preferences
    foreach ($mycourses_preferences as $preference => $class) {
...
// Override dark mode if darkmodefirst setting is enabled
if (theme_space_get_setting('darkmodefirst') == '1') {
    $extraclasses[] = 'theme-dark';
    $darkmodetheme = false;
}

// Add drawer open class if nav drawer is open and sidebar is visible
